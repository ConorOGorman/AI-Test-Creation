...
export default function Header() {
  ...
  const toggleMenu = () => {
    ...
    return (
    <>
      <header className="fixed top-0 left-0 w-[calc(100%-70px)] z-50 grid grid-cols-[170px_1fr_65px] gap-17.5 px-8.75 pt-8.75 pb-0 justify-center content-center transition-all duration-300 bg-transparent">
        <div className="relative self-center" id="header-logo">
          <Link href="/" className="text-primary-white block w-31.25">
        ...
        <div id="header-menu-items" className="uppercase justify-self-center self-center text-primary-white hidden lg:block">
           <ul className="overflow-hidden grid grid-flow-col auto-cols-max gap-8.75 list-none p-0.75 relative -left-[10%]">
        ...
        <div 
          ...
          onClick={toggleMenu}
          className="relative cursor-pointer flex justify-center items-center -right-4.375 gap-17.5 transition-transform duration-300 group"
          aria-label="Toggle Menu"
        >
          <div id="header-toggle-svg" className="w-8.75 h-4.75 flex flex-col justify-between">
             <span className="h-0.75 bg-primary-white w-full transition-transform duration-300 origin-center group-hover:scale-x-50"></span>
             <span className="h-0.75 bg-primary-white w-full transition-transform duration-300 group-hover:scale-x-200"></span>
             <span className="h-0.75 bg-primary-white w-full transition-transform duration-300 origin-center group-hover:scale-x-50"></span>
          </div>
        </div>
      </header>
      ...
      {/* Full Screen Menu */}
      <div className={`fixed inset-0 bg-primary-white z-15000 transition-opacity duration-700 ease-[cubic-bezier(0.56,0,0.38,1)] ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div id="full-menu-header" className="relative w-[calc(100%-70px)] grid grid-cols-[170px_1fr_1fr_50px] z-10 p-8.75 justify-center text-center items-center mx-auto">
            ...
            <div className="relative flex gap-8.75 overflow-hidden justify-self-end top-1.5">
            ...
            <div className="relative flex gap-8.75 overflow-hidden justify-self-center top-1.5">
                <span className="text-sm font-silka font-semibold uppercase">Make an appointment</span>
            </div>
            <div className="justify-self-end relative w-12.5 h-12.5 cursor-pointer flex justify-center items-center left-2.5" onClick={() => setIsMenuOpen(false)}>
                <div className="w-6.25 h-6.25 relative">
                    <span className="absolute top-1/2 left-0 w-full h-0.75 bg-primary-black rotate-45"></span>
                    <span className="absolute top-1/2 left-0 w-full h-0.75 bg-primary-black -rotate-45"></span>
                </div>
            </div>
        </div>
        ...
        <div id="full-menu-wrapper" className="relative w-full left-0 h-[calc(100vh-109px-50px)] overflow-x-hidden overflow-y-scroll z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 grid-rows-min">
            {['Kitchens', 'Interior Design', 'Projects', 'Services', 'Showrooms', 'About Us', 'Outlet', 'Contact'].map((item, index) => (
              <div key={item} className="relative p-8.75 pb-0 border-r border-border-light last:border-r-0">
